...
def execute_model(epochs=100):
    ...
    weight_input_hidden_layer = np.array(
        [[-0.424, -0.740, -0.961], [0.358, -0.577, -0.469]]
    )
    ...
    for epoch in range(epochs):
        ...
        sum_sinapse_in_hidden_layer = np.dot(inputs, weight_input_hidden_layer)

        # RESULTADO DA FUNÇÃO DE ATIVAÇÃO
        result_activation_function_in_hidden_layer = sigmoid(
            sum_sinapse_in_hidden_layer
        )
        ...
        sum_sinapse_out_hidden_layer = np.dot(
            result_activation_function_in_hidden_layer, weight_output_hidden_layer
        )

        # RESULTADO DA FUNÇÃO DE ATIVAÇÃO
        result_activation_function_out_hidden_layer = sigmoid(
            sum_sinapse_out_hidden_layer
        )
        ...
        absolute_error, mean_absolute_error = get_error(
            predict=result_activation_function_out_hidden_layer, y_true=y_true
        )
